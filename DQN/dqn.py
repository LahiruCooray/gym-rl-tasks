...
class DQNAgent:
    def __init__(self):
        ...
        self.target9_model.set_weights(self.model.get_weights())
    ...
    def train(self, terminal_state, step):
        ...
        # Get current states and next states from the minibatch
        ...
        # Update target model counter
        if terminal_state:
            self.target_update_counter += 1

        # Update target model every UPDATE_TARGET_EVERY steps
...
for episode in tqdm(range(1,EPISODES+1), ascii=True, unit='episode'):
    agent.tensorboard.step = episode  # Update tensorboard step
    episode_reward = 0
    step = 1
    # Reset environment
